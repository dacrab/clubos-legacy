...
import { CARD_DISCOUNT, EXTRA_SHOT_PRICE } from '@/lib/constants';
...
export function useSalesStore() {
  ...
  const addItem = useCallback((code: Code) => {
    setOrderItems((prev) => {
      ...
      const existingItem = prev.find((item) => item.code.id === code.id && !item.isTreat);
  ...
  const cartTotals = useMemo(() => {
    const subtotal = orderItems.reduce((sum, item) => {
      ...
      const dosageExtra =
        item.dosageCount && item.dosageCount > 1
