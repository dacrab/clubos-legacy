...
export function usePolling<T = void>({
    ...
    (error: unknown) => {
      ...
      const errorObj =
        error instanceof Error
          ? error
          : new Error(
              errorMessage || (typeof error === 'string' ? error : API_ERROR_MESSAGES.SERVER_ERROR)
            );
      if (onError) {
        onError(errorObj);
      }
  ...
  const pollFn = useCallback(async () => {
    ...
    try {
      ...
      const backoff =
        errorCountRef.current > 0
          ? Math.min(interval * 2 ** (errorCountRef.current - 1), maxBackoff)
          : interval;
