...
import { getVisibleNavItems, type NavigationItem } from './navigation.config';
...
export default function Sidebar({ role }: SidebarProps) {
  ...
  const canCloseRegister = visibleNavItems.some(
    item => item.href === '/dashboard/register-closings'
  );
      ...
      <Link
        ...
        className={cn(
          ...
          isActive
            ...
            : 'hover:bg-secondary/80 hover:shadow-soft hover:-translate-y-px'
        )}
      >
        <Icon
          className={cn(
  ...
  return (
    <div className="glass-effect shadow-soft fixed inset-y-0 left-0 z-50 hidden w-72 border-r lg:block">
      <div className="flex h-full flex-col">
        <div className="border-b p-6">
          <Link href="/dashboard" className="block">
            <h1 className="gradient-text text-2xl font-bold transition-transform hover:scale-[1.02]">
              Σύστημα Διαχείρισης
            </h1>
          </Link>
        </div>
        ...
        <nav className="flex-1 space-y-2 p-4">
          <div className="text-muted-foreground mb-4 px-4 text-sm font-semibold tracking-wider uppercase">
            Μενού
          </div>
