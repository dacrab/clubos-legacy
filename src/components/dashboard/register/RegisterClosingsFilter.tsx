...
function RegisterClosingsFilter({ onFilterChange }: RegisterClosingsFilterProps) {
  ...
  const formatDisplayDate = (from?: Date, to?: Date) => {
    ...
    if (!to) {
      return format(from, DATE_FORMAT.DISPLAY);
    }
    ...
    <div className="bg-muted/50 overflow-hidden rounded-lg border">
      <div className="flex items-center justify-between border-b px-4 py-3">
        ...
        <Button
          ...
          className="h-9 min-w-[100px] px-3 text-sm"
        >
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:flex lg:flex-wrap">
              ...
              <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    className={cn(
                      'mt-2 h-10 w-full justify-start text-left text-sm font-normal',
            ...
            <div className="mt-2 flex items-end sm:col-span-2 lg:col-span-1">
              <Button
                variant="outline"
                onClick={handleClear}
                className="flex h-10 items-center gap-2 px-4 text-sm"
              >
                <X className="h-4 w-4" />
                Καθαρισμός
              </Button>
