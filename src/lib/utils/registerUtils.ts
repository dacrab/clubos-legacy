 ...
 */
export const calculateStats = (sessions: ListItem[]): SessionStats =>
  sessions.reduce(
    (acc, session) => {
      if (session.type === 'closed') {
        ...
        if (session.orders) {
          session.orders.forEach(order => {
            const isCardPayment =
              Number((order as unknown as { card_discount_count?: number }).card_discount_count) >
              0;
            if (isCardPayment) {
              acc.totalCard += Number(
                (order as unknown as { final_amount?: number }).final_amount || 0
              );
            } else {
              acc.totalCash += Number(
                (order as unknown as { final_amount?: number }).final_amount || 0
              );
            }
          });
        ...
        session.orders.forEach(order => {
          const isCardPayment =
            Number((order as unknown as { card_discount_count?: number }).card_discount_count) > 0;
          if (isCardPayment) {
            acc.totalCard += Number(
              (order as unknown as { final_amount?: number }).final_amount || 0
            );
            acc.cardCount += Number(
...
export const calculateProductSummary = (orders?: Order[]): Record<string, ProductSummary> => {
  ...
  orders.forEach(({ sales = [] }) => {
    sales.forEach(sale => {
      ...
      if (isTreat) {
        summary[id].treatCount += quantity;
      }
...
export const calculateTransactionTotals = (orders?: Order[]): TransactionTotals => {
  ...
  return orders.reduce((acc, order) => {
    ...
    sales.forEach(sale => {
      if (!sale || (sale as ExtendedSale)?.is_deleted) {
        return;
      }
