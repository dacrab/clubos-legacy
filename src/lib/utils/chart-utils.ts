...
export function calculateSalesStats(sales: SaleWithDetails[]) {
  ...
  const cashSales = activeSales.filter(s => !s.isTreat && (!s.order?.cardDiscountCount || s.order.cardDiscountCount === 0));
  const cardSales = activeSales.filter(s => !s.isTreat && s.order?.cardDiscountCount && s.order.cardDiscountCount > 0);
  ...
  const cardDiscountCount = uniqueOrders.reduce((sum, order) => 
    sum + (order?.cardDiscountCount || 0), 0);
  ...
  const treatsAmount = +(treatSales.reduce((sum, sale) => 
    sum + +(parseFloat(sale.unitPrice) * sale.quantity).toFixed(2), 0)).toFixed(2);
  
  // Calculate final amounts after discounts with exact precision
  const totalBeforeDiscounts = +(nonTreatSales.reduce((sum, sale) => 
    sum + parseFloat(sale.totalPrice), 0)).toFixed(2);
  
  // Calculate cash and card revenues with exact precision
  const cashRevenue = +(cashSales.reduce((sum, sale) => sum + parseFloat(sale.totalPrice), 0)).toFixed(2);
  ...
  const cardRevenue = +((Object.values(salesByOrder) as SaleWithDetails[][]).reduce((totalRevenue: number, orderSales: SaleWithDetails[]) => {
    ...
    const orderTotal = +(orderSales.reduce((sum: number, sale: SaleWithDetails) => sum + parseFloat(sale.totalPrice), 0)).toFixed(2);
